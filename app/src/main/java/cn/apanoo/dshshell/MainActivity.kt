...
class MainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        recodeClipboardImage()   // v1.9.5：剪贴板图片转码保险（见方法注释）
    }

    /**
     * 剪贴板图片转码保险（v1.9.5）。
     *
     * 背景：截图/分享等非相册来源的剪贴板图片 URI，Chromium 粘贴时在 browser process
     * （= app 主进程）native 崩溃 → 无对话框直接退出（Java/渲染层都拦不住）。相册（MediaStore）
     * 复制的图片正常，说明崩溃与"外部图片 URI 的读取/解码路径"相关。
     *
     * 做法：app 回前台时若剪贴板含图片，后台线程把它读出 → 采样解码（最大边 2048 防 OOM）
     * → 重编码为自家 FileProvider 的 JPEG → 替换剪贴板。之后无论用户粘贴什么图片，
     * WebView 读到的都是自家可读的标准 JPEG，规避崩溃；粘贴图片功能完整保留。
     * 全程日志记录（URI 来源/尺寸/成败），失败时降级为清掉图片项（宁可不粘图也不崩）。
     */
    private fun recodeClipboardImage() {
        try {
            val cm = getSystemService(android.content.ClipboardManager::class.java)
            val clip = cm.primaryClip ?: return
            if (!clip.description.hasMimeType("image/*")) return   // 文本剪贴板不干预
            var srcUri: android.net.Uri? = null
            for (i in 0 until clip.itemCount) {
                val it = clip.getItemAt(i)
                if (it.uri != null && it.uri.scheme == "content") { srcUri = it.uri; break }
            }
            if (srcUri == null) return   // bitmap 直存型（罕见）不动
            val uri = srcUri
            CrashDiag.log(this, "recode start " + uri)
            val app = this
            Thread {
                try {
                    val resolver = contentResolver
                    ...
                    if (bounds.outWidth <= 0 || bounds.outHeight <= 0) {
                        CrashDiag.log(app, "recode FAIL badbounds ${bounds.outWidth}x${bounds.outHeight}")
                        dropClipboardImages(cm, app)
                        return@Thread
                    }
                    // 2) 采样解码（最大边 2048）
                    var sample = 1
                    ...
                    } ?: run { CrashDiag.log(app, "recode FAIL decode-null"); dropClipboardImages(cm, app); return@Thread }
                    // 3) 重编码 JPEG 到 cache/paste/
                    val dir = java.io.File(cacheDir, "paste").apply { mkdirs() }
                    val f = java.io.File(dir, "paste_${System.currentTimeMillis()}.jpg")
                    f.outputStream().use { bmp.compress(android.graphics.Bitmap.CompressFormat.JPEG, 88, it) }
                    bmp.recycle()
                    // 4) 自家 FileProvider URI 替换剪贴板
                    val fpUri = androidx.core.content.FileProvider.getUriForFile(this, "$packageName.fileprovider", f)
                    val nd = android.content.ClipData.newUri(resolver, "image/jpeg", fpUri)
                    cm.setPrimaryClip(nd)
                    CrashDiag.log(app, "recode ok ${bounds.outWidth}x${bounds.outHeight}->${f.length()}B")
                } catch (e: Exception) {
                    CrashDiag.log(app, "recode FAIL " + e.javaClass.simpleName + ": " + (e.message ?: ""))
                    dropClipboardImages(cm, app)
                }
            }.start()
        } catch (_: Exception) { }
    ...
    /** 转码失败时的降级：清掉剪贴板图片项（保留文本），避免粘贴崩溃。 */
    private fun dropClipboardImages(cm: android.content.ClipboardManager, app: MainActivity) {
        try {
            val clip = cm.primaryClip ?: return
            val texts = ArrayList<CharSequence>()
            for (i in 0 until clip.itemCount) {
                val t = clip.getItemAt(i).coerceToText(this)
                if (!t.isNullOrBlank()) texts.add(t)
            }
            if (texts.isEmpty()) cm.clearPrimaryClip()
            else cm.setPrimaryClip(android.content.ClipData.newPlainText("text", texts.joinToString("\n")))
            toast("剪贴板图片无法读取，已清除（避免粘贴崩溃）")
        } catch (_: Exception) { }
    }
